...
Solution Outline:.
1)  This is simple Java Stand alone app which reads three tab-seperated flat text files (One Vender file and two mapping files) and creates an output on the console (Console) based on values in the mapping files. 
2)	The Vendor file is in the following format : 
...
5)	The output file looks like the following :
...
7)	It also compares the ID;s in the vendor file with the ones in the ID mapping file and replaces all the vendor ID;s. Then it stores this result in a reqColList
...
5)	Finally, cd to target directory or copy over the JAR to the dir where you want to execute it. Copy the directory "/files" where all the three files (vendor.txt, idMpping.txt and columnMapping.txt) exists to the folder where
	the JAR is present/executed or the program wont run.(You can also create this folder and place these three files in there). 
